    ...
    void CreateGrid()
    {
        ...
        for(int x = 0; x < gridSizeX; x++)
        {
            for(int y = 0; y < gridSizeY; y++)
            {
                ...
                bool walk = !(Physics.CheckSphere(worldPos, nodeRadius,unwalkableMask));
                grid[x, y] = new Node(walk, worldPos);
            }
        }
    }
    ...
    void OnDrawGizmos()
    {
        ...
        if(grid != null)
        {
            foreach (Node node in grid)
            {
                ...
                Gizmos.DrawCube(node.m_worldPosition, Vector3.one *(nodeDiameter-0.1f));
            }
        }
    }
}
